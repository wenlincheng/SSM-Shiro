package com.wenlincheng.ssm.shiro.cache;
...
public class UserServiceAspect extends BaseCacheService{
    ...
    public Object userReadPointcut(ProceedingJoinPoint pjp) throws Throwable{
        ...
        TUser user = null;
        if(isId){
            user = (TUser)super.get(key);
        }else {
            // 先根据用户名从缓存中找到 id
            String idKey = idPrefix + super.get(key);
            user = (TUser) super.get(idKey);
        }
        if(user!=null){
            return user;
        }
     ...
     * @param rel
     */
    private void put(TUser rel){
        super.put(idPrefix + rel.getUserId(),rel);
        // 建立了一个 用户名前缀和 id 之间的关系
        super.put(usernamePrefix + rel.getUserName(),rel.getUserId());
    }
}
